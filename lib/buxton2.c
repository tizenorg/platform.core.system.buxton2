...
static void ref_noticb(struct bxt_noti_cb *noticb)
{
	noticb->ref_cnt++;
}

static GList *unref_noticb(GList *noti_cbs, struct bxt_noti_cb *noticb)
{
	noticb->ref_cnt--;
	if (noticb->ref_cnt == 0) {
...
static int proc_msg_noti(struct buxton_client *client, uint8_t *data, int len)
{
	...
	for (l = noti->callbacks, ll = g_list_next(l); l;
		...
		pthread_mutex_lock(&noti->cbs_lock);
		noti->callbacks = unref_noticb(noti->callbacks, noticb);
	}
